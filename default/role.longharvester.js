...
var role = {
    ...
    findContainerRoomName: function (creep) {
        let minCost;
        let res = null;
        _.filter(Game.rooms, r => r.memory.type == "my" && r.memory.pointPos && r.getPathToRoom(creep.memory.roomName) <= 3*50 && Game.map.getRoomLinearDistance(r.name, creep.memory.roomName) <= 3).forEach( function(r) {
            ...
            let carryDistance = r.getPathToRoom(creep.memory.roomName) || Game.map.getRoomLinearDistance(r.name, creep.memory.roomName) * 50 || 0;
