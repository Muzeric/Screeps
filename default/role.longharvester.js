...
var role = {
    run: function(creep) {
        if (Game.roomsHelper.getHostilesCount(creep.room.name) > 1 || (creep.memory.lastHostile && creep.pos.isBorder())) {
			creep.say("AAA");
			creep.moveTo(Game.spawns[creep.memory.spawnName].room.controller);
            creep.memory.lastHostile = 1;
			return;
        } else {
            creep.memory.lastHostile = 0;
        }
    ...
    findContainerRoomName: function (creep) {
        ...
        _.filter(Game.rooms, r => r.memory.type == "my" && r.memory.pointPos && Game.map.getRoomLinearDistance(r.name, creep.memory.roomName) <= 3).forEach( function(r) {
            ...
            let carryDistance = travel.getRoomsAvgPathLength(r.memory.pathCache, creep.memory.roomName) || Game.map.getRoomLinearDistance(r.name, creep.memory.roomName) * 50 || 0;
            let cost = (carryParts + carryDistance / 3) / r.energyCapacityAvailable + (r.memory.energy > MAX_ENERGY_BY_LONGHARVEST ? 50 : 0);
