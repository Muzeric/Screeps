...
var role = {
    run: function(creep) {
        ...
        if (creep.pos.isEqualTo(betweenPos)) {
            ...
            if (creep.carry[mineral.mineralType] > MINERALMINER_CARRY_LIMIT || creep.carry[mineral.mineralType] > 0 && !creep.pos.isEqualTo(container.pos) || creep.carry[mineral.mineralType] > 0 && creep.ticksToLive < KEEPLAIR_LEAVE_TIME * 2)
            ...
            if (global.cache.queueTransport.getStoreWithReserved(container, mineral.mineralType) >= TRANSPORTER_MIN_CONTAINER_AMOUNT)
                global.cache.queueTransport.addRequest(container, room.storage, mineral.mineralType, TRANSPORTER_MIN_CONTAINER_AMOUNT);
        } else {
